...
- [x] WEB-036：补齐资源包独立 lint、Stylelint 与依赖图/循环依赖门禁；让 `test:ui-labs:e2e` 自行构建所需 Lab，避免依赖调用顺序或现存 `dist/ui-labs`。
- [x] WEB-037：`createVue2Components` 改为必须显式传入 `include`（或默认空集），不再默认全量注册；Dialog/Shell 的可访问名称由调用方/i18n 注入，通用包仅保留安全默认。
- [x] WEB-038：按组件迁移进度删除 `UiPanel`/`UiSheet` 输出的 `glass/card/sheet` 兼容类、`main.js` 的旧 Liquid 全局注册和应用双轨样式；清点并消除未登记的 `!important` 与静态 inline style。Sass `@import` 迁移仍单独按[交接 TODO](Sass迁移TODO.md)执行。
